...
ways to implement load balancing~\cite{cardellini2002state}. For example, one
can  use round-robin DNS~\cite{brisco1995dns} to distribute the load to a set
of servers using  distinct IP addresses.  The DNS server will rotate the list
of addresses returned, causing clients to connect to different machines. DNS-
based load balancing has a number of drawbacks:   first, it is difficult to
add or remove web servers since clients will cache DNS information.  Second,
it requires that all servers have public  IP addresses so that they are
...
Reverse proxies (such as nginx~\cite{Reese:lj2008})  forward user requests to one or
...
a standalone \js runtime built on Google Chrome's V8 \js engine~\cite{v8} and it
...
\js uses a non-blocking I/O model based on an event queue.
...
Although event handlers run to completion, the order in which events are fired
...
it could be called synchronously. In Listing~\ref{code:nodefile} line 10 is
 executed before line 3 if \code{readFile}'s callback fires asynchronously. 
...
Listing~\ref{code:nodefile2}  lines 4--12 shows an incorrect solution that
...
lines 5--13 implement a barrier-style counter in vanilla \js, whereas 
lines 14--21 use async's \code{each} function, which provides parallel 
...
through outside providers, and maintains authentication state transparently
...
The login page itself is implemented by a virtual browser representing the
system's login application. The login page provides two authentication
options: a local authentication option where the user's credentials are stored
in a local database, for \cb supports local account creation and lookup.
Second, an OAuth~\cite{hardt2012oauth} option which authenticate users via
Google's OAuth authentication services.  In this option, the user clicks a
\emph{Login with Google Account} button which redirects to a Google account
authorization page asking the user to authorize \cb{} to access the user's
Google account information. After the user confirms Google will redirect the
user to \cb{}'s authentication callback URL to finalize the authentication.
...
\subsection{Application Instantiation Strategies}
...
\item[singleInstancePerUser] 
    ...
    for their application instances, in the same way singleBrowserPerUser instances do.
