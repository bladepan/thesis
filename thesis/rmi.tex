...
\label{ch:rmi}
\markright{Nodermi: A Remote Procedure Call Framework for \nodejs{}}
...
\code{callbackA} is created in \emph{Process B} because
it is not possible to copy function \code{callbackA} to \emph{Process B}.
The stub to \code{callbackA} is passed as an argument to
 \code{method1}.
Then \code{method1} invokes the stub to \code{callbackA} as if it were an ordinary
 local function and the actual \code{callbackA} is invoked in \emph{Process A}.
...
process A invokes \code{method1} with a function argument \code{callbackA}.
nodermi creates a \emph{method invocation message}
containing the object id of \code{objB} and the method being called.
The message also contains a description of the arguments:
the object id of \code{callbackA} and
the type of \code{callbackA}.
After process B receives the message,
it reconstructs the arguments for the method call:
creating stub \code{stubCallbackA} for \code{callbackA} since it is a function.
The stub \code{stubCallbackA} is a function and
it stores \code{callbackA}'s id and process A's process identifier.
After process B finds \code{objB} via its object id,
process B invokes \code{method1} with \code{stubCallbackA} as an argument.
\code{method1} then invokes \code{stubCallbackA} with \code{result},
which in turn initiates a remote method invocation
to call \code{callbackA} in process A.
...
the caller needs to hold a reference to \code{callbackA}
...
that ensures that the entry to \code{callbackA} is deleted when the
