...
after \code{method1} of \code{stubB} is called,
\code{callbackA} is out of scope in process A,
it is not referenced anymore by the user's code.
If nodermi does not hold reference to \code{callbackA},
it could be garbage collected before it is invoked later
by \emph{process B}.
In fact, in practice callback functions like \code{callbackA} are almost
...
A process could terminate without 
