...
When a proxy process accepts a client it parse the incoming HTTP request to extract the application
id, which is part of the request's path.  Each proxy process maintains a table of known mappings from 
...
Nodermi takes advantage of this knowledge and handle methods with callback parameter 
transparently~\footnote{A method with a callback parameter is not necessarily a asynchronous method,
